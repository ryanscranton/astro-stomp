...
//type:circle or square (0 or 1) 
//radius/area: radius of cirlce or -99 if polygon
//weight: pixel weight
//ra1 dec1: position cirlce center or first vertex of polygon
//the remaining verticies of the polygon if type is 1 will be listed ra dec and can be arbitrarly long
  ...
  while (!region_file.eof()) {
    ...
    for (std::vector<std::string>::iterator coord=coordinates.begin();
	 coord!=coordinates.end();++coord,++i) {
      if (i==0) {
	idx = std::strtol(coord->c_str(),NULL, 10);
      } 
      else if (i==1) {
	type = std::strtol(coord->c_str(),NULL, 10);
      } 
      else if (i==2) {
	radius = std::strtod(coord->c_str(),NULL);
      }
      else if (i==3) {
	weight = std::strtod(coord->c_str(),NULL);
      }
      else if (i>3) {
	...
	//else {
	//  bound = 
	//    new Stomp::CircleBound(Stomp::AngularCoordinate(ra[0],dec[0],
	...
	if (FLAGS_verbose) {
	  ...
	  std::cout << "type: " << type << ", radius: " << radius << ", weight: " << weight <<
	    ", Area: " << bound->Area() << 
	    ", Lambda: " << bound->LambdaMin() << " - " << bound->LambdaMax() << 
	    ", Eta:" << bound->EtaMin() << " - " << bound->EtaMax() << "\n";
	}
	Stomp::Map* tmp_map = 
	  new Stomp::Map(*bound,weight,FLAGS_max_resolution,FLAGS_verbose);
	n_kept++;
	
	pixelized_raw_area += tmp_map->Area();
	raw_area += bound->Area();
	if (FLAGS_add_maps) 
	  exp_map->AddMap(*tmp_map,false);
	else
	  exp_map->IngestMap(*tmp_map);
