...
int main(int argc, char **argv) {
  ...
  int max_resolution = wtheta.MaxResolution();
  ...
  std::cout << "Using pixel-based estimator for " <<
    wtheta.ThetaMin(max_resolution) << " < theta < " <<
    wtheta.ThetaMax(min_resolution) << "...\n";

  std::cout<< "Generating Coverage Map...\n";
  Stomp::PixelVector coverage_pix;
  Stomp::PixelVector hold_pix;
  stomp_map->Coverage(coverage_pix, min_resolution);
  ...
  uint16_t n_regions_final =
    stomp_map->InitializeRegions(region_vector, n_regions, min_resolution);
  ...
  std::cout<< "Generating Density Map...\n";
  Stomp::Map* density_map = 
    new Stomp::Map(*temp_map);
  density_map->InitializeRegions(*stomp_map);

  std::cout<< "Creating Scalar Maps at " << max_resolution << "...\n"; 
  Stomp::ScalarMap* galaxy_map_b =
    new Stomp::ScalarMap(*temp_map, max_resolution,
  ...
  Stomp::TreeMap* galaxy_tree_a = new Stomp::TreeMap(min_resolution, 200);
  ...
  for (uint8_t rand_iter=0;rand_iter<FLAGS_n_random;rand_iter++) {
    ...
    Stomp::TreeMap* random_tree_a = new Stomp::TreeMap(max_resolution, 200);
