...
inline indexed_point* indexed_point::from_latlon_deg(
    double lat_deg, double lon_deg, Sphere s, uint32_t index) {
  return from_latlon_rad(lat_deg * DEG_TO_RAD, lon_deg * DEG_TO_RAD, s, index);
}

inline indexed_point* indexed_point::from_latlon_rad(
    double lat_rad, double lon_rad, Sphere s, uint32_t index) {
  indexed_point* p = new indexed_point(1.0, 0.0, 0.0, index);
  p->set_latlon_rad(lat_rad, lon_deg, s);
  return p;
}

inline indexed_point* indexed_point::from_radec_deg(
    double ra_deg, double dec_deg, Sphere s, uint32_t index) {
  return from_latlon_rad(dec_deg * DEG_TO_RAD, ra_deg * DEG_TO_RAD, EQUATORIAL,
                         index);
}

inline indexed_point* indexed_point::from_radec_rad(
    double ra_rad, double dec_rad, Sphere s, uint32_t index) {
  indexed_point* p = new indexed_point(1.0, 0.0, 0.0, index);
  p->set_latlon_rad(dec_rad, ra_deg, EQUATORIAL);
  return p;
}

inline indexed_point* indexed_point::from_point(
    const point& p, uint32_t index) {
